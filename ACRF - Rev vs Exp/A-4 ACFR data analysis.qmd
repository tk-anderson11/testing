...
# Conditional test for equality (using a tolerance for floating-point comparison)
...
plot3 <- ggplot(federal_rev_data, aes(x = year, y = amount)) +
  ...
  labs(title = "Federal Contracts and Grants Over the Last 10 Years",
       x = "Year",
       y = "Billions of $") +
  theme_minimal() +
  scale_y_continuous(labels = function(x) x/1e9)
...
```{r}
## Pie chart of 2024 revenue for fun :)
...
## RShiny percentage change calculator!

```{r}
# Assume revenue_data is already created

ui <- fluidPage(
  titlePanel("Revenue Data Analysis"),

  sidebarLayout(
    sidebarPanel(
      selectInput("line_item", "Select Line Item:",
                  choices = unique(revenue_data$line_item)),
      width = 3
    ),
...
server <- function(input, output) {
  ...
  year_data <- reactive({
    filtered_data() %>%
      ...
      arrange(desc(year)) %>%
      mutate(
        previous_year_amount = lag(amount, default = first(amount)),
        percentage_change=((previous_year_amount - amount)/previous_year_amount)*100
      ) %>%
      ungroup() %>%
      mutate(percentage_change = sprintf("%.2f%%", percentage_change)) %>%
...
output$year_plot <- renderPlotly({
  ...
  p <- ggplot(plot_data, aes(x = year,  # Use year directly if it's already numeric
                           y = percentage_change_numeric, # use the converted numeric column
                           text = paste("Year: ", year, "<br>Percentage Change: ", percentage_change))) +
    geom_line(color = "blue") +  # Specify the color for the line here
    geom_point(color = "blue") + # Match the point color to the line
...
```{r}
other_fin_data <- joint_data_long %>% 
  filter(type =="Other Financing Sources (Uses)") %>% 
  select(-footnote) #%>% 
  #filter(!is.na(amount)) ## chose to filter out NAs?

ggplot(other_fin_data, aes(x = year, y = amount, color = line_item)) +
  geom_point() +  # Or geom_point() for a scatterplot, geom_col() for bar chart
  labs(title = "Other Financing Sources (Uses) Over Time",
       x = "Date",
       y = "Amount") +
  theme_minimal()
```
...
```{r}
other_fin_data %>% 
  filter(line_item == "Sale of Capital Assets") %>% 
  ggplot(x = year, y = amount)+
  geom_line(aes(x = year, y = amount), color ="blue") +
  labs(
    title = "Sale of Capital Assets Over Time",
    x = "Year",
    y = "Amount (Millions of $)" # Changed y-axis label
  ) +
  scale_x_continuous(breaks=seq(min(other_fin_data$year), max(other_fin_data$year), by = 1),labels = as.integer(seq(min(other_fin_data$year), max(other_fin_data$year), by = 1))) +
...
# filter(line_time == "Net Change in Fund Balances")
```

## Debt Service Percentage

```{r}
# pull in debt service numbers from initial upload
...
debt_service_percentage <- as.numeric(gsub("%","",debt_service_percentage_str))/100
...
ggplot(data, aes(x = year, y = DebtServicePercentage, group = 1)) +
  geom_line() +
  geom_point() +
